<?php
  ...
  $site_nome = 'Meio Norte';
  $site_link = 'http://www.meionorte.com';

  $html = file_get_html($site_link.'/');

  //Manchete
  foreach($html->find('div.destaque-manchete') as $article) { $i++;
      $item['title']     = ( $article->find('a h1', 0)->plaintext );
      $item['link']     = $article->find('a', 0)->href;
      $articles[] = $item['title'] != '' ? $item : '';
  }

  //Slideshow
  foreach($html->find('#slides li') as $article2) { $i++;
      $item['title']     = ( $article2->find('a h3', 0)->plaintext );
  ...
  foreach($articles as $noticia){
    ...
    $link = isset($link_aux['host']) ? $link : $site_link.$link;
